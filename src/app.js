...
scanButton.addEventListener('click', async () => {
    ...
    try {
        if (!navigator.bluetooth) {
            updateBluetoothStatus('Web Bluetooth is not supported on this browser or device.');
            alert('Web Bluetooth is not supported on this browser or device. Try Chrome on Desktop/Android over HTTPS or localhost.');
            return;
        }
        ...
        updateBluetoothStatus(`Connecting to "${bluetoothDevice.name || 'Unnamed Device'}"...`);

        // Connect to the GATT server
        gattServer = await bluetoothDevice.gatt.connect();
        if (gattServer.connected) {
            updateBluetoothStatus(`Connected to "${bluetoothDevice.name || 'Unnamed Device'}"!`, true);
            ...
            try {
                const deviceInformationService = await gattServer.getPrimaryService('device_information');
                const modelNumberCharacteristic = await deviceInformationService.getCharacteristic('model_number_string');
                ...
                console.warn('Could not read Device Information Service/Characteristic:', error);
                characteristicData.textContent += 'Device Information Service not found or accessible.\n';
            }
...
function onDisconnected(event) {
    ...
    updateBluetoothStatus(`Disconnected from "${device.name || 'Unnamed Device'}"`);
